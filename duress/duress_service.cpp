#define LOG_TAG "vendorDuressService"
...
int main() {
    ...
    LOG(INFO) << "duress_wipe service started";
    ...
    if (wipeSuccess != 0) {
        LOG(INFO) << "Titan M wipe failed";
    }

    const std::string citadeldInTestModeProp = "vendor.citadeld.duress.mode";
    const std::string testProp = android::base::GetProperty(citadeldInTestModeProp, "");
    ...
    LOG(WARNING) << citadeldInTestModeProp << " prop is set running in test mode";
